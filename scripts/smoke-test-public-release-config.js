...
check(packageJson.version === '1.0.12', `package version is ${packageJson.version}`);
check(packageLock.version === '1.0.12', `lockfile version is ${packageLock.version}`);
check(packageLock.packages?.['']?.version === '1.0.12', `lockfile root package version is ${packageLock.packages?.['']?.version}`);
...
check(indexSource.includes('Version: v1.0.12'), 'initial version label is not 1.0.12');
